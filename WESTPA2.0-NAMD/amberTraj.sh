...
iter=$(head iters.txt -n 1 | awk '{ print $1 }')
seg=$(head iters.txt -n 1 | awk '{ print $2 }')

echo Iteration=$siter
echo Segment=$sseg
seg=$(head iters.txt -n 1 | awk '{ print $2 }')echo Segment=$ssegi of the relevant simulation and print the iteration and segment IDs of all
# successful trajectories.  This information is stored and will be used later on.
#w_succ -W for_analysis/west.h5 >> succ.txt
#python python.py
...
mkdir $dir
cp ../common_files/step5_input.psf $dir
...
cat output.dat | tail -n +7 > iters.txt
...
cpptraj -p step5_input.psf -i cpptraj.in > traj.log
